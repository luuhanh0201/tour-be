...
export const findAllTourModel = async ({ q = "", limit = 10, page = 1 } = payload) => {
    q = String(q ?? "").trim();
    ...
    const [rows] = await poolConnection.query(sqlData, params)
...
export const findTourByNameModel = async ({ tourName }) => {
    ...
    const [rows] = await poolConnection.query(sql, [tourName]);
...
export const findTourByCodeModel = async ({ code }) => {
    ...
    const [rows] = await poolConnection.query(sql, [code]);
    const tour = rows[0] || null;
    return {
        exist: !!tour,
        tour: tour
...
export const findTourByIdModel = async ({ tourId }) => {
    ...
    const [rows] = await poolConnection.query(sql, [tourId]);
    const tour = rows[0] || null;
    return {
        exist: !!tour,
        tour: tour
...
export const addNewTourModel = async (payload = {}) => {
    ...
    const [row] = await poolConnection.query(sql, [code, name, categoryId, durationDays, durationNights, description, highlights, basePrice, status])
    return row || null
}
